#!/usr/bin/env python3
"""
Build a static HTML site for the AI Models knowledge graph.
Usage: python3 build_ai_models.py
"""
...
.type-模型{background:#0EA5E9}
.type-概念{background:#8B5CF6}
.type-组织{background:#059669}
.type-人物{background:#DC2626}
.type-基准测试{background:#D97706}
.type-方法{background:#7C3AED}
.type-索引{background:#64748B}
...
def build_sidebar_html(files, current_stem=""):
    ...
    for cat in ["models", "concepts", "organizations", "people", "benchmarks", "methods"]:
    ...
    for cat, label in order:
        ...
        is_open = " open" if (cat == "models" or contains_current) else ""
    ...
    has_changelog = any(f["stem"] == "更新日志" and f["category"] == "concepts" for f in files)
    if has_changelog:
        changelog_active = " active" if current_stem == "更新日志" else ""
        html += f'<a href="{BASE_PATH}/concepts/更新日志.html" class="nav-link nav-changelog{changelog_active}">更新日志</a>\n'
...
<aside class="sidebar">
  <div class="sidebar-header"><a href="{BASE_PATH}/index.html" class="logo">AI 模型知识图谱</a></div>
...
def build_homepage(files, link_map, ref_counts):
    ...
    model_ranked = sorted(model_files, key=lambda f: ref_counts.get(f["stem"], 0), reverse=True)[:15]
    ...
    org_files = [f for f in files if f["category"] == "organizations"]
    org_ranked = sorted(org_files, key=lambda f: ref_counts.get(f["stem"], 0), reverse=True)[:12]

    people_files = [f for f in files if f["category"] == "people"]

    benchmark_files = [f for f in files if f["category"] == "benchmarks"]

    sources_total = len(model_files)

    people_data = {
        "Sam Altman":         ("S", "OpenAI CEO"),
        "Dario Amodei":       ("D", "Anthropic CEO"),
        "Ilya Sutskever":     ("I", "SSI 联合创始人"),
        "Yann LeCun":         ("Y", "Meta 首席 AI 科学家"),
        "Andrej Karpathy":    ("A", "前 OpenAI/Tesla AI 负责人"),
        "Demis Hassabis":     ("D", "Google DeepMind CEO"),
        "Jensen Huang":       ("J", "NVIDIA CEO"),
        "Fei-Fei Li":         ("F", "斯坦福 HAI 联合主任"),
        "Geoffrey Hinton":    ("G", "深度学习之父"),
        "Yoshua Bengio":      ("Y", "Mila 创始人"),
    }
...
<section class="hero-section">
  <div class="hero-eyebrow">AI Models · Knowledge Graph</div>
  <h1 class="hero-title">AI 模型<span class="gold">知识图谱</span></h1>
  <p class="hero-sub"><b>{len(model_files)} 个</b>模型，<b>{len(concept_files)} 个</b>核心概念，<b>{len(method_files)} 个</b>方法<br>
  从 Transformer 到 GPT-5——追踪 2023-2026 年大模型技术全景</p>
</section>

<div class="stats-row">
  <a href="{BASE_PATH}/models/GPT-4o.html" class="stat-item"><div class="stat-num">{len(model_files)}</div><div class="stat-label">AI 模型</div></a>
  <a href="{BASE_PATH}/concepts/Transformer.html" class="stat-item"><div class="stat-num">{len(concept_files)}</div><div class="stat-label">核心概念</div></a>
  <a href="{BASE_PATH}/methods/RLHF.html" class="stat-item"><div class="stat-num">{len(method_files)}</div><div class="stat-label">技术方法</div></a>
  <a href="{BASE_PATH}/benchmarks/MMLU.html" class="stat-item"><div class="stat-num">{len(benchmark_files)}</div><div class="stat-label">基准测试</div></a>
</div>

<div class="main-inner">
<div class="nav-cards">
  <a href="{BASE_PATH}/models/GPT-4o.html" class="nav-card">
    <span class="nav-card-icon">🤖</span>
    <div class="nav-card-title">模型</div>
    <div class="nav-card-sub">{len(model_files)} 个主流 AI 模型</div>
    <span class="nav-card-arrow">{arrow}</span>
  </a>
  <a href="{BASE_PATH}/concepts/Transformer.html" class="nav-card">
    <span class="nav-card-icon">💡</span>
    <div class="nav-card-title">核心概念</div>
    <div class="nav-card-sub">{len(concept_files)} 个基础概念</div>
    <span class="nav-card-arrow">{arrow}</span>
  </a>
  <a href="{BASE_PATH}/methods/RLHF.html" class="nav-card">
    <span class="nav-card-icon">🔬</span>
    <div class="nav-card-title">技术方法</div>
    <div class="nav-card-sub">{len(method_files)} 个训练与推理方法</div>
    <span class="nav-card-arrow">{arrow}</span>
  </a>
  <a href="{BASE_PATH}/benchmarks/MMLU.html" class="nav-card">
    <span class="nav-card-icon">📊</span>
    <div class="nav-card-title">基准测试</div>
    <div class="nav-card-sub">{len(benchmark_files)} 个评测基准</div>
    <span class="nav-card-arrow">{arrow}</span>
  </a>
  <a href="{BASE_PATH}/organizations/OpenAI.html" class="nav-card">
    <span class="nav-card-icon">🏢</span>
    <div class="nav-card-title">组织</div>
    <div class="nav-card-sub">{len(org_files)} 个核心机构</div>
    <span class="nav-card-arrow">{arrow}</span>
  </a>
  <a href="{BASE_PATH}/people/Sam Altman.html" class="nav-card">
    <span class="nav-card-icon">👤</span>
    <div class="nav-card-title">关键人物</div>
    <div class="nav-card-sub">{len(people_files)} 位核心人物</div>
    <span class="nav-card-arrow">{arrow}</span>
  </a>
</div>
...
<div class="section">
  <div class="section-header">
    ...
    <span class="section-count">TOP {len(model_ranked)}</span>
  </div>
  <div class="tag-cloud">
'''
    for i, f in enumerate(model_ranked):
...
<div class="section">
  <div class="section-header">
    ...
    <div class="section-line"></div>
    <span class="section-count">TOP ''' + str(len(concept_ranked)) + '''</span>
  </div>
  <div class="tag-cloud">
'''
...
<div class="section">
  <div class="section-header">
    <h2 class="section-title">技术方法</h2>
    <div class="section-line"></div>
    <span class="section-count">''' + str(len(method_ranked)) + ''' 个</span>
  </div>
  <p style="font-size:14px;color:var(--text2);margin:-8px 0 16px;font-family:var(--serif)">大模型训练、对齐与推理的核心技术方法。</p>
  <div class="tag-cloud">
'''
...
<div class="section">
  <div class="section-header">
    <h2 class="section-title">核心组织</h2>
    <div class="section-line"></div>
    <span class="section-count">TOP ''' + str(len(org_ranked)) + '''</span>
  </div>
  <div class="tag-cloud">
'''
    for i, f in enumerate(org_ranked):
        url = link_map.get(f["stem"], "#")
        count = ref_counts.get(f["stem"], 0)
        tier = " tier-1" if i < 3 else (" tier-2" if i < 6 else "")
        html += f'    <a href="{url}" class="tag{tier}">{f["stem"]}<span class="tag-n">{count}</span></a>\n'
...
<div class="footer-promo">
  <div class="promo-story">
    <h3>关于本站</h3>
    <p>把散落在各家官方博客、技术报告、研究论文、Hugging Face 模型卡中的 AI 模型信息，整理成一张可以漫游的知识图谱。覆盖 OpenAI、Anthropic、Google、Meta、阿里、DeepSeek 等主要厂商，从 2023 年 GPT-4 到 2026 年 GPT-5，跨越大模型黄金三年。</p>
    <p>核心视角：<strong>模型能力对比</strong>、<strong>架构演进</strong>、<strong>训练方法论</strong>、<strong>开源 vs 闭源生态</strong>——追踪 AI 模型如何从聊天机器人进化为通用智能体。</p>
    <p class="promo-credit">数据更新至 2026 年 6 月 3 日。由 <strong>Claude Code</strong> 构建。</p>
  </div>
</div>
...
def main():
    ...
    assets_src = Path(__file__).parent / "assets-anthropic"
